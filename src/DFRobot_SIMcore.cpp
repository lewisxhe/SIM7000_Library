...
static HardwareSerial *DFSIMSerial;

void DFRobot_SIMcore::begin(HardwareSerial &s_)
{
    DFSIMSerial = &s_;
}
...
bool DFRobot_SIMcore::check_send_cmd(const char *cmd, const char *resp, unsigned int timeout, unsigned int chartimeout )
{
    char SIMbuffer[100];
...
{
    DFSIMSerial->write((uint8_t *)buff, num);
}
...
{
    DFSIMSerial->updateBaudRate(rate);
}
